...
import 'package:programming_sns/features/chat/providers/old/chat_users_provider.dart';
...
class ChatScreen2 extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _ChatScreen2State();
}

class _ChatScreen2State extends ConsumerState<ChatScreen2> {
    ...
    return Scaffold(
      ...
      body: ref.watchEX(
        ...
        complete: (currentUserModel) {
          ...
          final messages = ref.watch(messagesProvider(widget.chatRoomId)).value;

          if (chatUsers == null || messages == null) {
            return const Center(
  ...
  Future<void> onSendTap(String message, ReplyMessage replyMessage, MessageType messageType) async {
    ...
    if (updateMessage == null) {
      ...
      // メッセージ送信
      await _messagesNotifier.upsertState(msg);
