...
class _ChatScreenState extends ConsumerState<ChatScreen2> {
          ...
          complete: (currentUserModel) {
            ...
            return ref.watchEX(
              ...
              complete: (chatController) {
                _chatController2 = chatController;

                /// EVENT
                ref.watch(chatMessageEventProvider(widget.chatRoomId));

                chatController.initialMessageList.forEach(
                  (element) {
                    print('メッセージタイプ：画面:${element.messageType}');
                  },
                );
                // _chatController = chatController;
                // _chatController.initialMessageList =Data. messageList;
                return ChatView(
  ...
  Future<void> _onSendTap(
    ...
    // _chatController2.addMessage(msg);
